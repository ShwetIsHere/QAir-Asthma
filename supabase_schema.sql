...
-- Inhaler Count Table (stores remaining doses for each user)
CREATE TABLE IF NOT EXISTS inhaler_count (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- Create index for faster queries
CREATE INDEX IF NOT EXISTS idx_inhaler_count_user_id ON inhaler_count(user_id);

-- Enable Row Level Security
ALTER TABLE inhaler_count ENABLE ROW LEVEL SECURITY;

-- Create policies for inhaler_count
CREATE POLICY "Users can view own inhaler count"
  ON inhaler_count
  FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own inhaler count"
  ON inhaler_count
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own inhaler count"
  ON inhaler_count
  FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own inhaler count"
  ON inhaler_count
  FOR DELETE
  USING (auth.uid() = user_id);

-- Function to auto-update updated_at timestamp
CREATE OR REPLACE FUNCTION update_inhaler_count_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER inhaler_count_updated_at
  BEFORE UPDATE ON inhaler_count
  FOR EACH ROW
  EXECUTE FUNCTION update_inhaler_count_updated_at();
